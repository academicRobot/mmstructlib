...
void *
mycalloc (const char *filename, const int linenr, size_t nelem, size_t elsize)
{
	int *ip;
    ...
    double **lidots, int *nu_dots)
{
	...
	j = wkbox[0];
	for (i=1; i<nxyz; i++) j= MAXIMUM(wkbox[i], j);
		...
		for (iy=0; iy<nybox; iy++)
		{
			...
			for (ix=0; ix<nxbox; ix++)
			{
				...
				for (iat=iii1; iat<iii2; iat++)
				{
                    ...
                    //nnei = number of atoms intersecting atom i_at
                    //wknb = data structure of distances and dot product from intersecting atoms to i_at
					...
					if (nnei)
					{
						last = 0;
						i_ac = 0;
						for (l=0; l<n_dot; l++) //for each dot, assume obscured unless none of the overlapping atoms obscure it
						{
                            //xus - coordinates? of sphere dots
                            //This 'if' short circuits the inner loop: check last successful match against next dot
							if (xus[3*l]*(wknb+last)->x+
							        xus[1+3*l]*(wknb+last)->y+
							        xus[2+3*l]*(wknb+last)->z <= (wknb+last)->dot)
							{
								for (j=0; j<nnei; j++)  //last match didn't obscure, check all intersecting atoms
								{
								...
								if (j >= nnei) //none obscure, mark as accessible
								{
									i_ac++;
									wkdot[l] = 1;
								}
